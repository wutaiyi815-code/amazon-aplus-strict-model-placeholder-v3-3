...
def main() -> int:
    ...
    parser.add_argument("--background-root", required=True)
    ...
    for product_dir in sorted([p for p in root.iterdir() if p.is_dir()]):
        ...
        product_images = images_in(product_dir)
        ...
        front_ref = tagged_identity or choose_front_reference(product_images)
