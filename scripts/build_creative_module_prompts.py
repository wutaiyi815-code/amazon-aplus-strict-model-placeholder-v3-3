...
def extract_product_summary(global_context: str) -> str:
    ...
    gender_match = re.search(
        r"(?im)^\s*[-*]?\s*(?:target_gender|target gender)\s*:\s*(women|men|unisex)\s*$",
        global_context,
    )
    source_match = re.search(
        r"(?im)^\s*[-*]?\s*target_gender_source\s*:\s*model_identity_reference\s*$",
        global_context,
    )
    if gender_match and source_match:
        # The visible model is authoritative. Do not pass a conflicting Excel
        # target_customer value into the image API as if it controlled casting.
        customer_prefixes = ("target_customer", "target customer", "zielgruppe")
        lines = [
            line
            for line in lines
            if not line.lstrip("- ").casefold().startswith(customer_prefixes)
        ]
        lines.append(
            f"- Target gender: {gender_match.group(1).casefold()} "
            "(authoritative from the selected model identity reference)"
        )
        lines.append("- Attribute-table target customer is advisory only and must not override this model gender.")
    if lines:
        return "\n".join(lines[:28])
...
def product_signal_blob(global_context: str) -> str:
    model_gender_is_authoritative = bool(
        re.search(
            r"(?im)^\s*[-*]?\s*target_gender_source\s*:\s*model_identity_reference\s*$",
            global_context,
        )
    ...
    facts = [
        ...
        "" if model_gender_is_authoritative else product_fact(global_context, "target_customer"),
...
def render_callouts_block(callouts: str) -> str:
    """Discard legacy callout content; final prompts use Headline and Copy only."""
    return ""
