...
SECTION3_HUMAN_ACTION_MARKER = "Section 3 fashion action:"
    ...
    for key, value in raw.items():
        ...
        tag = {
            ...
            "detail_types": [
                ...
                for item in value.get("detail_types", []) if str(item).strip()
            ],
...
def is_model_identity_image(path: Path, identity_anchor: Path | None = None) -> bool:
    return tag_contains_person(path) is True and str(model_view_tag(path).get("subject_type") or "") == "model_person"
...
def classify_references(product_dir: Path, identity_anchor: Path | None = None) -> dict[str, list[Path]]:
    images = [path for path in product_images(product_dir) if str(model_view_tag(path).get("subject_type") or "") != "brand_logo"]
    model_refs = sorted(
...
def image_role(path: Path, identity_anchor: Path | None = None) -> str:
    ...
    if subject == "model_person":
        return {"front": "model_front", "three_quarter": "model_side", "side": "model_side", "back": "model_back"}.get(view, "model_unknown")
    if subject == "product_flatlay":
        return {"front": "product_front", "back": "product_back"}.get(view, "product_flatlay_other")
...
def role_sources(
    ...
    sources["model_identity"] = unique([*sources["model_front"], *sources["model_side"]], 999)
    if identity_anchor is not None:
    ...
    sources["product_detail"] = unique([
        *sources["product_detail"], *sources["fabric_closeup"],
        *sources["construction_detail"], *sources["graphic_detail"],
    ], 999)
    return sources
...
def detect_module_role(context: str, index: int) -> tuple[str, list[str]]:
    ...
    influencer_background_matches = [keyword for keyword in ROLE_KEYWORDS["influencer_background"] if keyword.lower() in normalized]
    ...
    multi_angle_terms = ("多角度", "正侧背", "正面侧面背面", "front side back", "front-side-back", "multi-angle", "multiple angles")
    ...
    if any(term in normalized for term in model_terms) and any(term in normalized for term in flatlay_terms) and any(term in normalized for term in front_back_terms):
        return "model_flatlay", sorted({term for term in (*model_terms, *flatlay_terms, *front_back_terms) if term in normalized})
    if any(term in normalized for term in model_terms) and any(term in normalized for term in multi_angle_terms):
        return "model_multi_angle", sorted({term for term in (*model_terms, *multi_angle_terms) if term in normalized})
    if any(term in normalized for term in detail_terms) and not context_requests_model(context):
        return "construction_detail", sorted({term for term in detail_terms if term in normalized})
    if any(term in normalized for term in hero_terms) and any(term in normalized for term in model_terms):
        return "hero_model", sorted({term for term in (*hero_terms, *model_terms) if term in normalized})
    if any(term in normalized for term in model_terms) and any(term in normalized for term in fit_terms):
        return "fit_silhouette", sorted({term for term in (*model_terms, *fit_terms) if term in normalized})
    if any(term in normalized for term in front_back_terms):
        return "front_back", sorted({term for term in front_back_terms if term in normalized})
    scores: dict[str, int] = {}
    matched: list[str] = []
...
def validate_semantic_tag_coverage(product_dir: Path) -> None:
    ...
    invalid_source = sorted(
        key for key in allowed & tagged
        if not str(MODEL_VIEW_TAGS[key].get("source") or "").strip()
        or str(MODEL_VIEW_TAGS[key].get("source") or "").casefold() == "heuristic"
    )
    if missing or stale or invalid_source:
        raise RuntimeError(
...
def source_priority(product_dir: Path, path: Path) -> tuple[int, str]:
    ...
    rank = 0 if top == "素材".casefold() else 1 if top in {"上身", "上身1"} else 2 if top == "aigc" else 3
...
def validate_selected_reference(product_dir: Path, path: Path) -> Path:
    ...
    if relative_key(product_dir, resolved) not in MODEL_VIEW_TAGS:
...
def select_dynamic_contract_refs(
    ...
    if module_role == "no_reference":
        ...
        required, optional = {"model_front": 1}, {"model_side": 1}
    elif module_role == "model_multi_angle":
        required, optional = {"model_front": 1, "model_back": 1}, {"model_side": 1}
    elif module_role == "fit_silhouette":
        required, optional = {"model_front": 1}, {"model_side": 1, "model_back": 1}
    elif module_role == "model_flatlay":
        required, optional = {"model_front": 1, "product_front": 1, "product_back": 1}, {"model_side": 1, "product_detail": 1}
    elif module_role == "front_back":
        required, optional = {"product_front": 1, "product_back": 1}, {"product_detail": 2}
    elif module_role in {"construction_detail", "fabric_detail", "graphic_detail"}:
        required, optional = {"verified_detail": 1}, {"verified_detail": 4}
    elif module_role in {"placeholder_blocks", "influencer_background"}:
        required, optional = {"product_front": 1}, {"product_detail": 2, "brand_logo": 1}
    elif module_role == "influencer":
        raise RuntimeError(f"{product_dir.name}: influencer references must be rebuilt by the influencer workflow, not preserved from history")
    else:
    ...
    pools["verified_detail"] = sorted_sources(product_dir, [
        *sources.get("fabric_closeup", []), *sources.get("construction_detail", []),
        *sources.get("graphic_detail", []), *sources.get("product_detail", []),
    ])
    ...
    for role, count in required.items():
        ...
        selected_roles[role] = len(chosen)
        if len(chosen) < count:
            missing.append(f"{role}:{count - len(chosen)}")
    if missing:
        raise RuntimeError(f"{product_dir.name}: module contract {module_role} missing required semantic roles: {', '.join(missing)}")
    ...
    return selected, {
        "required_roles": required, "optional_roles": optional,
        "selected_roles": selected_roles, "contract_validation": "passed",
    }
...
def enforce_section_two_identity_lock(
    ...
    companion = next(
        (
            ...
            and path_has_directory(path, "AIGC")
            and is_preferred_identity_view(path)
        ),
        None,
    )
...
def model_identity_keys(sources: dict[str, list[Path]]) -> set[str]:
    return {model_key(path) for path in sources.get("model_identity", [])}
...
def enforce_no_single_back_view_model_ref(
    ...
    """Allow a back view only when a confirmed front/three-quarter person accompanies it."""
    model_keys = model_identity_keys(sources)
    if not model_keys:
        return refs

    model_refs = [path for path in refs if model_key(path) in model_keys]
    back_refs = [path for path in model_refs if is_back_view_model(path)]
    if not back_refs:
        return refs
    if any(is_preferred_identity_view(path) for path in model_refs):
        return refs
    ...
    companion = next(
        (
            path
            for path in sources.get("model_identity", [])
            if model_key(path) not in existing_keys and is_preferred_identity_view(path)
        ),
        None,
    )
    if companion is not None:
    ...
    # A side/unknown person is not a valid identity lock for a back view. Keep
    # any non-back model references, but remove every back view when no visible
    # front or three-quarter identity companion exists.
    return unique([path for path in refs if path not in back_refs], max_refs)
...
def validate_section3_human_fashion_action(prompt_path: Path) -> tuple[bool, str]:
    """Validate a main-agent-authored action without generating or rewriting it."""
    text = read_text(prompt_path)
    legacy = [marker for marker in LEGACY_SECTION3_ACTION_MARKERS if marker.casefold() in text.casefold()]
    if legacy:
        return False, f"remove legacy scripted action block: {legacy[0]}"
    marker_count = text.casefold().count(SECTION3_HUMAN_ACTION_MARKER.casefold())
    if marker_count != 1:
        return False, f"expected exactly one '{SECTION3_HUMAN_ACTION_MARKER}' block, found {marker_count}"
    pattern = re.compile(
        rf"(?ims)^\s*{re.escape(SECTION3_HUMAN_ACTION_MARKER)}\s*(.*?)"
        r"(?=^\s*(?:Product Accuracy|Produktgenauigkeit|Text / Callouts|Negative Prompt|"
        r"No LOGO rule|Keine LOGO-Regel|Reference note|Attached Reference Image Map)\s*:|\Z)"
    )
    match = pattern.search(text)
    action = re.sub(r"\s+", " ", match.group(1)).strip(" -") if match else ""
    if len(action) < 90:
        return False, "human fashion action is missing or too short; describe body action, gaze/torso direction, and product-clearance logic"
    ...
    if body_term_count < 2:
        return False, "human fashion action needs at least two concrete body/action cues"
    return True, action
...
def main() -> int:
    ...
    for product_dir in sorted(path for path in root.iterdir() if path.is_dir()):
        ...
        # Always rebuild from a deliberately empty map; historical paths are never inherited.
        ...
        for prompt_path in prompt_paths:
            ...
            authoritative_context = "\n".join(str(module_meta.get(key, "")) for key in ("title", "instruction", "raw"))
            ...
            if module_role == "unresolved":
                raise RuntimeError(f"{product_dir.name}/{prompt_path.name}: cannot derive module function from template and prompt semantics")
            refs, contract = select_dynamic_contract_refs(
                product_dir, module_role, sources, identity_anchor, args.max_references_per_module
            )
            needs = list(contract["required_roles"]) + list(contract["optional_roles"])
