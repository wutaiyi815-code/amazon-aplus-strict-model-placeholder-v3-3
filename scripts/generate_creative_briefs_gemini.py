...
def hard_excluded(path: Path, product_dir: Path) -> bool:
    for part in path.relative_to(product_dir).parts[:-1]:
        ...
        if folded.startswith(("_aplus", "_influencer")) or folded in {name.casefold() for name in HARD_EXCLUDED_DIR_NAMES}:
            return True
...
def candidate_model_reference(product_dir: Path, images: list[Path]) -> Path | None:
    ...
    if tag_path.exists():
        ...
        for rel, tag in tags.items():
            ...
            person_confirmed = contains_person is True and subject_type == "model_person"
            if (
                not path.exists()
...
def model_gender_from_tags(product_dir: Path, identity: Path | None, tags: dict[str, Any]) -> str:
    if identity is None:
        return "unspecified"
    identity_key = str(identity.resolve()).casefold()
    for rel, tag in tags.items():
        if not isinstance(tag, dict):
            continue
        candidate = (product_dir / str(rel)).resolve()
        if str(candidate).casefold() != identity_key:
            continue
        value = str(tag.get("gender_presentation") or "unknown").strip().casefold().replace("-", "_")
        if value in {"woman", "women", "female", "female_presenting"}:
            return "women"
        if value in {"man", "men", "male", "male_presenting"}:
            return "men"
        if value in {"ambiguous", "androgynous", "unisex"}:
            return "unisex"
    return "unspecified"
...
def reference_images_for_request(product_dir: Path, images: list[Path], contact_sheet: Path) -> list[Path]:
    ...
    product_refs = [
        p for p in images if p.relative_to(product_dir).parts[0].casefold() == "素材"
    ]
...
def build_prompt(
    ...
    identity_gender = model_gender_from_tags(product_dir, identity, tags)
    tag_excerpt = json.dumps(tags, ensure_ascii=False, indent=2)[:10000]
    return f"""You are the product-analysis and creative-copy stage for the amazon-aplus-strict-model-placeholder-v3-3 workflow.
...
AUTHORITATIVE MODEL-IMAGE GENDER
{identity_gender}
...
def enforce_model_gender(text: str, target_gender: str) -> str:
    """Make the selected model tag authoritative even if the analysis model echoes Excel gender."""
    if target_gender == "unspecified":
        return text
    replacements = {
...
def main() -> int:
    ...
    for product_dir in product_dirs(root):
        ...
        brief = enforce_model_gender(brief, model_gender_from_tags(product_dir, identity, tags))
