...
def load_module_reference_map(product_dir: Path) -> dict[str, list[Path]]:
    ...
    for module_name, image_paths in data.items():
        if isinstance(image_paths, list):
            mapping[str(module_name)] = [validate_current_sku_reference(product_dir, Path(str(item)).expanduser()) for item in image_paths]
    return mapping
...
def is_hard_excluded_reference(path: Path, product_dir: Path) -> bool:
    ...
    for part in parts:
        ...
        if folded.startswith(SKIP_DIR_PREFIXES) or folded in {name.casefold() for name in HARD_EXCLUDED_DIR_NAMES}:
            return True
...
def validate_current_sku_reference(product_dir: Path, image_path: Path) -> Path:
    candidate = image_path if image_path.is_absolute() else product_dir / image_path
    ...
    if not resolved.is_file() or resolved.suffix.lower() not in IMAGE_EXTENSIONS:
        raise RuntimeError(f"Reference does not exist in current SKU: {resolved}")
...
def has_model_reference_images(product_dir: Path, image_paths: list[Path], role_map: dict[str, str]) -> bool:
    model_roles = {"model_identity", "model_front", "model_side", "model_back"}
...
def all_model_reference_paths(product_dir: Path, role_map: dict[str, str]) -> list[Path]:
    ...
    for path in sorted(product_dir.rglob("*"), key=lambda item: str(item).casefold()):
        ...
        if any(part.lower().startswith(("_aplus", "_influencer")) for part in relative_parts):
            continue
        if is_hard_excluded_reference(path, product_dir):
            continue
        if reference_role(product_dir, path, role_map) in {"model_identity", "model_front", "model_side", "model_back"}:
            paths.append(path)
...
def logo_references(product_dir: Path) -> list[Path]:
    ...
    for path in sorted(product_dir.rglob("*"), key=lambda item: str(item.relative_to(product_dir)).lower()):
        if path.is_file() and path.suffix.lower() in IMAGE_EXTENSIONS and "logo" in path.stem.lower() and not is_hard_excluded_reference(path, product_dir):
            logos.append(path)
    return logos
...
def strip_text_callouts(prompt: str) -> str:
    """Remove every legacy Text / Callouts block before API submission."""
    cleaned = re.sub(
        r"(?ims)^[ \t]*Text[ \t]*/[ \t]*Callouts[ \t]*:[ \t]*[^\r\n]*(?:\r?\n|\Z).*?"
        r"(?=^[ \t]*(?:Reference note|Referenzhinweis|No LOGO rule|Keine LOGO-Regel|Logo Reference Requirement|"
        r"LOGO-Referenz|No reference images rule|Keine Referenzbilder-Regel|No-model reference rule|"
        r"Negative Prompt|Layout execution|Section 3 fashion action)[ \t]*:|\Z)",
        "",
        prompt,
    )
    return re.sub(r"\n{3,}", "\n\n", cleaned).strip()
...
def strip_none_callouts(prompt: str) -> str:
    """Backward-compatible alias for the all-callouts submission cleanup."""
...
def generate_one_module(
    ...
    prompt = strip_text_callouts(prompt)
    ...
    for attempt in range(1, len(DEFAULT_BACKOFF) + 2):
        try:
            ...
            body = response.json()
            task_id = body.get("taskId") or body.get("id") or body.get("task_id")
            ...
            if provider == PROVIDER_RH:
                result_url, poll_body = poll_rh_generation(requests_mod, api_key, task_id, poll_timeout, queued_timeout)
                append_api_event(
                    product_dir,
                    prompt_path.name,
                    {"event": "generation_completed", "provider": provider, "task_id": task_id, "result_url": result_url, "response_body": poll_body},
                )
            else:
                ...
                append_api_event(
                    product_dir,
                    prompt_path.name,
                    {"event": "generation_completed", "provider": provider, "task_id": task_id, "result_url": result_url},
                )
...
def main() -> int:
    ...
    parser.add_argument("--product", help="Optional product folder name, or comma-separated names, to generate only selected products.")
    ...
    for product_dir in product_dirs:
        ...
        if set(module_reference_map) != prompt_names:
            raise RuntimeError(
                f"{product_dir.name}: module reference map must be freshly rebuilt with exact prompt coverage; "
                f"missing={sorted(prompt_names - set(module_reference_map))}, stale={sorted(set(module_reference_map) - prompt_names)}"
            )
        ...
        for prompt_path in prompts:
            ...
            module_reference_paths = module_reference_map[prompt_path.name]
            ...
            if module_needs_logo:
                product_logo_paths = product_logo_paths if product_logo_paths is not None else logo_references(product_dir)
                extra_paths = unique_paths(extra_paths + product_logo_paths)
                if product_logo_paths:
                    ...
                    print("  LOGO requested by prompt, but no matching LOGO image was found.")
            ...
            planned_paths = [validate_current_sku_reference(product_dir, path) for path in planned_paths]
            ...
            print("  Using freshly rebuilt module-specific references; shared recursive fallback is disabled.")
            generate_one_module(
                requests,
                api_key,
                args.provider,
