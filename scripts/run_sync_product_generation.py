...
def main() -> int:
    ...
    command = [sys.executable, str(generator), "--root", str(root), "--template-analysis", str(template), "--provider", args.provider, "--product", args.product, "--size", args.size, "--resolution", args.resolution, "--quality", args.quality, "--max-reference-images", str(args.max_reference_images)]
