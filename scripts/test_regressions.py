...
class CalloutRemovalTests(unittest.TestCase):
    def test_all_callouts_are_omitted_by_builder(self) -> None:
    ...
    def test_api_submission_strips_real_callouts(self) -> None:
        prompt = "Headline:\nWarm Layers\n\nText / Callouts:\nRibbed cuffs\nBrushed interior\n\nNegative Prompt:\nrandom text"
        cleaned = generator.strip_text_callouts(prompt)
        self.assertNotIn("Text / Callouts", cleaned)
        self.assertNotIn("Ribbed cuffs", cleaned)
        self.assertIn("Headline:\nWarm Layers", cleaned)
        self.assertIn("Negative Prompt:\nrandom text", cleaned)

    def test_nested_negative_prompt_survives_callout_cleanup(self) -> None:
        prompt = "Text / Callouts:\nNegative Prompt: No mismatched colors.\n\nReference note:\nKeep identity."
        cleaned = generator.strip_text_callouts(prompt)
        self.assertNotIn("Text / Callouts", cleaned)
        self.assertIn("Negative Prompt: No mismatched colors.", cleaned)
...
class ModelReferenceClassificationTests(unittest.TestCase):
    ...
    def test_section_two_forces_brief_anchor_and_front_companion(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            reference_builder.MODEL_VIEW_TAG_PRODUCT_DIR = product
            reference_builder.MODEL_VIEW_TAGS = {
                "aigc/front.png": {"view": "front", "usable_for_identity": True, "contains_person": True, "subject_type": "model_person"},
                "aigc/three-quarter.png": {"view": "three_quarter", "usable_for_identity": True, "contains_person": True, "subject_type": "model_person"},
                "aigc/back.png": {"view": "back", "usable_for_identity": False, "contains_person": True, "subject_type": "model_person"},
            }
    ...
    def test_section_two_companion_must_come_from_aigc(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            product = Path(temp_dir) / "SKU"
            (product / "AIGC").mkdir(parents=True)
            (product / "上身").mkdir()
            identity = product / "AIGC" / "identity.png"
            aigc_companion = product / "AIGC" / "three-quarter.png"
            upbody_person = product / "上身" / "front-person.jpg"
            for path in (identity, aigc_companion, upbody_person):
                path.write_bytes(b"test")
            reference_builder.MODEL_VIEW_TAG_PRODUCT_DIR = product
            reference_builder.MODEL_VIEW_TAGS = {
                "aigc/identity.png": {"view": "front", "usable_for_identity": True, "contains_person": True, "subject_type": "model_person"},
                "aigc/three-quarter.png": {"view": "three_quarter", "usable_for_identity": True, "contains_person": True, "subject_type": "model_person"},
                "上身/front-person.jpg": {"view": "front", "usable_for_identity": True, "contains_person": True, "subject_type": "model_person"},
            }
            sources = {"model_identity": [upbody_person, aigc_companion, identity]}
            selected, diagnostics = reference_builder.enforce_section_two_identity_lock(
                [], sources, identity, 6
            )
            self.assertEqual(selected[:2], [identity.resolve(), aigc_companion.resolve()])
            self.assertNotIn(upbody_person.resolve(), selected)
            self.assertTrue(diagnostics["section2_identity_companion_added"])

    def test_back_view_requires_front_or_three_quarter_companion(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            product = Path(temp_dir) / "SKU"
            (product / "AIGC").mkdir(parents=True)
            back = product / "AIGC" / "back.png"
            side = product / "AIGC" / "side.png"
            front = product / "AIGC" / "front.png"
            for path in (back, side, front):
                path.write_bytes(b"test")
            reference_builder.MODEL_VIEW_TAG_PRODUCT_DIR = product
            reference_builder.MODEL_VIEW_TAGS = {
                "aigc/back.png": {"view": "back", "usable_for_identity": False, "contains_person": True, "subject_type": "model_person"},
                "aigc/side.png": {"view": "side", "usable_for_identity": True, "contains_person": True, "subject_type": "model_person"},
                "aigc/front.png": {"view": "front", "usable_for_identity": True, "contains_person": True, "subject_type": "model_person"},
            }
            sources = {"model_identity": [back, side, front]}
            selected = reference_builder.enforce_no_single_back_view_model_ref(
                [back, side], sources, 6
            )
            self.assertIn(front.resolve(), selected)
            self.assertIn(back.resolve(), selected)

            without_front = {"model_identity": [back, side]}
            selected = reference_builder.enforce_no_single_back_view_model_ref(
                [back, side], without_front, 6
            )
            self.assertNotIn(back.resolve(), selected)
            self.assertIn(side.resolve(), selected)
...
class Section3HumanFashionActionTests(unittest.TestCase):
    def test_overlap_requires_main_agent_action_block(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("expected exactly one", reason)
    ...
    def test_legacy_safe_library_block_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            valid, reason = reference_builder.validate_section3_human_fashion_action(prompt)
            self.assertFalse(valid)
            self.assertIn("legacy scripted action", reason)
    ...
    def test_detailed_human_action_is_validated_without_rewrite(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            original = (
                "Visual Direction:\nModel plus flat lays.\n\n"
                "Section 3 fashion action:\n"
                "Angle the torso toward the product cards while the right hand lightly gathers the jacket collar and the left arm extends downward outside the graphic area; direct the gaze across the layout so the shoulder line creates movement without hiding the zipper or hem.\n\n"
                "Product Accuracy:\nExact product.\n"
            )
...
class ModelAuthoritativeGenderTests(unittest.TestCase):
    ...
    def test_gemini_output_gender_is_overridden_by_model_tag(self) -> None:
        draft = (
            "## Target And Language\n"
            "- target_gender: women\n"
            "- target_gender_source: attribute_table_fallback\n"
        )
        enforced = briefs.enforce_model_gender(draft, "men")
        self.assertIn("- target_gender: men", enforced)
        self.assertIn("- target_gender_source: model_identity_reference", enforced)

    def test_api_product_summary_omits_conflicting_target_customer(self) -> None:
        context = (
            "## Product Truth\n"
            "- Product type: hoodie\n"
            ...
            "## Target And Language\n"
            "- target_gender: men\n"
            "- target_gender_source: model_identity_reference\n"
        )
        summary = builder.extract_product_summary(context)
        self.assertNotIn("Target customer: Women", summary)
        self.assertIn("Target gender: men", summary)
        self.assertIn("authoritative from the selected model identity reference", summary)
