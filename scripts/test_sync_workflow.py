...
class ContactSheetTagTests(unittest.TestCase):
    ...
    def test_validate_tags_requires_exact_coverage_and_normalizes_back_identity(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            raw = {"I001": {
                "contains_person": True, "subject_type": "model_person", "view": "back",
                "framing": "full_body", "usable_for_identity": True, "identity_quality": "low",
                "gender_presentation": "woman", "notes": "rear view",
            }}
            result = tags.validate_tags(raw, manifest, product, "gpt-test")
            tag = next(iter(result.values()))
